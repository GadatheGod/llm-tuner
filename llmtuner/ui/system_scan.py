...
class SystemScanTab(QWidget):
    ...
    def _do_scan(self):
        ...
        from llmtuner.core.system_info import get_ram_info, get_disk_info, get_ram_speed
    ...
    def _update_cpu_section(self):
        ...
        for i, (label, val) in enumerate(pairs):
            grid.addWidget(QLabel(label), i, 0, Qt.AlignLeft | Qt.AlignTop)
            v = QLabel(val)
            v.setStyleSheet("font-weight: bold; color: #2d5f8a;")
            grid.addWidget(v, i, 1, Qt.AlignLeft)

        group.setLayout(grid)
        self._add_section_widget(group, 0, 0, 1, 2)
    ...
    def _update_gpu_section(self):
        ...
        for idx, gpu in enumerate(gpus):
            ...
            group.setLayout(grid)
            self._add_section_widget(group, idx + 1, 0, 1, 2)
        ...
        if not gpus:
            ...
            self._add_section_widget(group, 1, 0, 1, 2)
    ...
    def _update_ram_section(self):
        ...
        group.setLayout(grid)
        self._add_section_widget(group, len(self.system_info.gpu) + 1, 2, 1, 2)
    ...
    def _update_disk_section(self):
        ...
        pairs = [
            ...
            ("Type", self.system_info.disk_type),
            ("OS", f"{self.system_info.os_name} {self.system_info.os_version} ({self.system_info.os_arch})"),
        ]
        for i, (label, val) in enumerate(pairs):
            grid.addWidget(QLabel(label), i, 0, Qt.AlignLeft | Qt.AlignTop)
            v = QLabel(str(val))
        ...
        group.setLayout(grid)
        self._add_section_widget(group, len(self.system_info.gpu) + 2, 0, 1, 2)

    def _add_section_widget(self, widget, row, col, row_span, col_span):
